...
export const actions = {
	assignTicket:/** @param {import('./$types').RequestEvent} event */  async ({ request, params }) => {
		const { user } = await locals.auth.validateUser();
	...
	pickupTicket:/** @param {import('./$types').RequestEvent} event */  async ({ locals, params }) => {
		const { user } = await locals.auth.validateUser();
	...
	openTicket:/** @param {import('./$types').RequestEvent} event */  async ({ locals, params }) => {
		const { user } = await locals.auth.validateUser();
	...
	closeTicket:/** @param {import('./$types').RequestEvent} event */  async ({ locals, params }) => {
		const { user } = await locals.auth.validateUser();
	...
	note:/** @param {import('./$types').RequestEvent} event */  async ({ request, locals, params }) => {
		const { user } = await locals.auth.validateUser();
		const form = await request.formData();
